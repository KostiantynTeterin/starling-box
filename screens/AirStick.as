package screens 
{
	...
	public class AirStick extends Screen
	{
		...
		private function _oef(e:Event):void 
		{
			if ( DIR8_CONTROL ) {
				if (joy.RG) {
					ship.x += MAX_SPEED * joy.speed_factor;
				}
				...
				if (joy.UP) {
					ship.y -= MAX_SPEED * joy.speed_factor;
				}						
				...
				if ( !joy.NONE) {				
					ship.x += Math.cos( joy.angle ) * (MAX_SPEED * joy.speed_factor);
					ship.y += Math.sin( joy.angle ) * (MAX_SPEED * joy.speed_factor);
				}
			}
			// --
			if (joy.NONE) {
			...
			if ( ship.x	< 0 ) ship.x += SB.width;
			if ( ship.x	> SB.width ) ship.x -= SB.width;
			if ( ship.y	< 0 ) ship.y += SB.height;
			if ( ship.y	> SB.height ) ship.y -= SB.height;						
