	...
	*/
	public class ColorLerp
	{
		...
		public static function lerp( c1:uint, c2:uint, amt:Number ):uint
		{
			...
			{
				if ( a1 > 0 || a2 > 0 )
					return uint(lerp( a1, a2, amt )) << 24 | lerpRGB( c1 >> 16 & 0xff, c1 >> 8 & 0xff, c1 & 0xff, c2 >> 16 & 0xff, c2 >> 8 & 0xff, c2 & 0xff, amt ); 
				else
			...
			else
			{
				...
				if ( a1 > 0 || a2 > 0 )
					return uint(lerp( a1, a2, amt )) << 24 | lerpHSV( e1.h, e1.s, e1.v, e2.h, e2.s, e2.v, amt );
				else
		...
		public static function gradient( c1:uint, c2:uint, step:uint ):Array
		{
			...
			if ( _rgb_mode )
			{
				...
				{
					for ( i = 1; i <= step; i++ )
						grad[i] = uint(lerp( a1, a2, i * amt )) << 24 | lerpRGB( r1, g1, b1, r2, g2, b2, i * amt );
				}
				else
				{
					for ( i = 1; i <= step; i++ )
			...
			else
			{
				...
				{
					for ( i = 1; i <= step; i++ )
						grad[i] = uint(lerp( a1, a2, i * amt )) << 24 | lerpHSV( h1, s1, v1, h2, s2, v2, i * amt );
				}
				else
				{
					for ( i = 1; i <= step; i++ )
		...
		{
			return uint(lerp( r1, r2, amt )) << 16 | uint(lerp( g1, g2, amt )) << 8 | uint(lerp( b1, b2, amt ));
		}
		...
		{
			return FColor.HSVtoValue( lerp( h1, h2, amt ), lerp( s1, s2, amt ), lerp( v1, v2, amt ) );
		}
		 ...
		 */
		private function lerp( a:Number, b:Number, amt:Number ):Number
		{
			return a + (b - a) * amt;
		}
